...
class ConsoleWindow
{
    ...
    //TODO: Should layers be able to have their own sub-layers, which in turn
    //could have even more sub-layers? I can see some pretty interesting things
    //with this. If not, just change protected to private. ;-)
    protected ConsoleLayer[] layers;
    protected XY size;
    protected Slot[][] slots;

    protected void sld(XY pos) @safe @nogc nothrow pure
    {
        auto line = pos.y;
        auto col = pos.x;
    ...
    Slot getSlot(XY location) const
    {
    ...
    Slot[][] snap() const
    {
        ...
        auto snap = new typeof(return)(slots.length, slots[0].length);
        foreach (y, col; snap)
            col[] = slots[y][];

        //Magic pasta-code which returns a 'snap'.
        foreach (const ref layer; layers)
        {
            if (layer.visible)
            {
                foreach (x; 0 .. layer.size.x)
                {
                    foreach (y; 0 .. layer.size.y)
                    {
                        if (!layer.visible ||
                            layer.getSlot(XY(x, y)).character == ' ' &&
                            layer.getSlot(XY(x, y)).background == bg.init &&
                            layer.getSlot(XY(x, y)).mode != md.swap &&
                            layer.transparent)
                            continue;
                        ...
                        version (Windows)
                        {
                            if (layer.getSlot(XY(x, y)).color == fg.init)
                                layer.setSlotColor(XY(x, y), fg.white);
                            if (layer.getSlot(XY(x, y)).background == bg.init)
                                layer.setSlotBackground(XY(x, y), bg.black);
                        }

                        snap[y + layer.location.y][x + layer.location.x] = layer.getSlot(XY(x, y));
                    }
                }
            }
        }
    ...
    */
    void addLayer(ConsoleLayer...)(ConsoleLayer consoleLayers) @safe pure nothrow @nogc
    {
...
class ConsoleLayer : ConsoleWindow
{
    ...
    @property @safe pure @nogc nothrow
    {
    ...
    */
    override Slot getSlot(XY location) const @safe pure @nogc nothrow
    {
    ...
    void setSlotBackground(XY xy, bg background)
    {
        ...
        {
            clayersLog("Warning: Failed to set background ", background,
                " at ", xy.x, ", ", xy.y, ")");
        }
    }
...
private
{
    ...
    version (Windows)
    {
        ...
        void scp(XY pos)
        {
            ...
            COORD c = {
                cast(short) min(info.srWindow.Right - info.srWindow.Left + 1, max(0,
                    pos.x)), cast(short) max(0, pos.y)};
                stdout.flush();
